<?php session_start(); ?>
     <style type="text/css">
		...
		#stud{
          ...
          <h1 class="page-header">Liste des résultats des étudiants      <button type="text" class="btn btn-info" onclick="printContent('stud')" >
...
<form action="" method="post">
      ...
      <button class="btn btn-success" type="submit" name ="submit">Tout voir</button>
      <label for="focusedInput">Selecionner niveau:</label>
      <select name="getselect" class="form-control" style="height:30px;font-size:12px" >
        <option > </option>
    <?php 
    include 'db.php';
    $query=mysqli_query($conn,"SELECT * FROM school_year Order by SY_ID");
